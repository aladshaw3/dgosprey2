 ...
 *
 *				dD/dt = x*(C)^m*[k_r*(D)^x*product(j, C_j^v_j) - k_f*(L)^n*product(i, C_i^v_i)
 *
...
class CoupledCatalyst : public CoupledConstChemisorption
{
public:
    /// Required constructor for objects in MOOSE
    CoupledCatalyst(const InputParameters & parameters);
...
protected:
    ...
    std::vector<Real> _cat_stoich;                      ///< Catalyst stoichiomentry

    std::vector<const VariableValue *> _coupled_cat;	///< Pointer list to catalyst concentrations (mol/L)
