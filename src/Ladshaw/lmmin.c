 ...
 */
{
	//Add the ability to print iterations to a file
//	FILE *iterations;
//	iterations = fopen("Iterations.txt","w+");
    int i;
...
/*
    if( printflags & 1 )
    {
            ...
            if (iflag == 2)
            {
                fprintf(iterations,"trying step in gradient direction  ");
            } else if (iflag == 1)
            {
                fprintf(iterations,"determining gradient (iteration %2d)", iter);
            } else if (iflag == 0)
            {
                fprintf(iterations,"starting minimization              ");
            } else if (iflag == -1)
            {
                fprintf(iterations,"terminated after %3d evaluations   ", nfev);
            }
    }

    if( printflags & 2 )
    {
        fprintf(iterations,"  par: ");
        for (i = 0; i < n_par; ++i)
            fprintf(iterations," %18.11g", par[i]);
        fprintf(iterations," => norm: %18.11g", lm_enorm(m_dat, fvec));
    }
    ...
    {
        fprintf(iterations,"  residuals:\n");
        for (i = 0; i < m_dat; ++i)
            fprintf(iterations,"    fvec[%2d]=%12g\n", i, fvec[i] );
    }
    fclose(iterations);
*/
//-----------------------------------------------------------------
