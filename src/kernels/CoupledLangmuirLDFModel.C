...
registerMooseObject("dgospreyApp", CoupledLangmuirLDFModel);

template<>
InputParameters validParams<CoupledLangmuirLDFModel>()
{
	InputParameters params = validParams<CoupledLangmuirModel>();
...
CoupledLangmuirLDFModel::CoupledLangmuirLDFModel(const InputParameters & parameters)
: CoupledLangmuirModel(parameters),
...
void CoupledLangmuirLDFModel::computeLDFcoeff()
{
	CoupledLangmuirModel::computeLangmuirCoefficient();
	
	double part_coef = 1.0;
	double Henry = (_maxcap * _langmuircoef );
...
void CoupledLangmuirLDFModel::computeScalingFactor()
{
	_scaling_factor = (_alpha*(1.0-(_u[_qp]/_maxcap)) + _beta*(_u[_qp]/_maxcap));
}

Real CoupledLangmuirLDFModel::computePartCoeffTempDerivative()
{
	return _maxcap*_pellet_density[_qp]*_langmuircoef*_enthalpy/Rstd/_coupled_temp[_qp]/_coupled_temp[_qp];
}

Real CoupledLangmuirLDFModel::computeLDFoffdiag()
{
	double part_coef = 1.0;
	double Henry = (_maxcap * _langmuircoef );
	part_coef = _pellet_density[_qp]*Henry;
...
Real CoupledLangmuirLDFModel::computeLDFjacobian()
{
	return _ldf_coeff*_phi[_j][_qp]*( (_beta/_maxcap) - (_alpha/_maxcap) );
}

Real CoupledLangmuirLDFModel::computeQpResidual()
{
	computeLDFcoeff();
	computeScalingFactor();
	return _ldf_coeff*_scaling_factor*CoupledLangmuirForcingFunction::computeQpResidual() + _test[_i][_qp] * _u_dot[_qp];
}

Real CoupledLangmuirLDFModel::computeQpJacobian()
{
	computeLDFcoeff();
	computeScalingFactor();
...
Real CoupledLangmuirLDFModel::computeQpOffDiagJacobian(unsigned int jvar)
{
	computeLDFcoeff();
	computeScalingFactor();
	...
	{
		return -_test[_i][_qp]*_ldf_coeff*_scaling_factor*CoupledLangmuirForcingFunction::computeLangConcDerivative();
	}
	
	// Off-diagonal element for coupled temperature
	if (jvar == _coupled_var_temp)
	{
		return -_test[_i][_qp]*_ldf_coeff*_scaling_factor*CoupledLangmuirModel::computeLangTempDerivative() + _test[_i][_qp]*_scaling_factor*computeLDFoffdiag()*(_u[_qp] - CoupledLangmuirForcingFunction::computeLangmuirEquilibrium());
	}
