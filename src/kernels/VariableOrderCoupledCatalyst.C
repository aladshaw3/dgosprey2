/*!
 *  \file VariableOrderCoupledCatalyst.C
...
{
	InputParameters params = validParams<CoupledCatalyst>();

    params.addParam< std::vector<Real> >("catalyst_order","Reaction order for each catalyst species");
    params.addParam< std::vector<Real> >("gas_order","Reaction order for each gas species");
    params.addParam< std::vector<Real> >("ads_order","Reaction order for each adsorbed/deactivated species");
    params.addParam< std::vector<Real> >("site_order","Reaction order for each adsorbent species");

	return params;
}

VariableOrderCoupledCatalyst::VariableOrderCoupledCatalyst(const InputParameters & parameters)
: CoupledCatalyst(parameters),
_c_order(getParam<std::vector<Real> >("catalyst_order")),
_g_order(getParam<std::vector<Real> >("gas_order")),
_a_order(getParam<std::vector<Real> >("ads_order")),
_s_order(getParam<std::vector<Real> >("site_order"))
{

}

Real VariableOrderCoupledCatalyst::computeRateFunction()
{
...
Real VariableOrderCoupledCatalyst::computeRateFunctionJacobi()
{
...
Real VariableOrderCoupledCatalyst::computeRateFunctionGasOffDiagJacobi(int i)
{
...
Real VariableOrderCoupledCatalyst::computeRateFunctionCatalystOffDiagJacobi(int i)
{
...
Real VariableOrderCoupledCatalyst::computeRateFunctionAdsOffDiagJacobi(int i)
{
...
Real VariableOrderCoupledCatalyst::computeQpResidual()
{
    return _test[_i][_qp]*_u_dot[_qp] - _test[_i][_qp]*computeRateFunction();
}

Real VariableOrderCoupledCatalyst::computeQpJacobian()
{
    return _test[_i][_qp]*_phi[_j][_qp]*_du_dot_du[_qp] - _test[_i][_qp]*computeRateFunctionJacobi();
}

Real VariableOrderCoupledCatalyst::computeQpOffDiagJacobian(unsigned int jvar)
{
